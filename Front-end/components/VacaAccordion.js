	...
	return (
		<Accordion>
			<AccordionSummary 
				...
				id="compensation"
			>
				<Typography>{kind[category]}</Typography>
			</AccordionSummary>
			<AccordionDetails>
				<Typography>
				...
				{records.map((el, idx)=>(
					<div key={idx} style={{fontSize:'16px', color:"#777"}}>{el.num_of_days}일 : {el.reason}</div>
				))}				
				...
				{(edit===true) && (
					...
					>
						<Grid item md={4.5}>
       					...
       					clearOnEscape
       					renderInput={(params) => (
	         				<TextField {...params} label="받은 휴가 일수" variant="standard" />
  	     				)}
								value={day}
						...
						<Grid item md={1}>
							<IconButton sx={{marginTop:2}} onClick={()=>editVa(category, content, day.id)}>
								<AddIcon />
							</IconButton>
						</Grid>
					</Grid>
